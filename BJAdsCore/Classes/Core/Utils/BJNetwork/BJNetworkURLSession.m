...
@interface BJNetworkURLSession () <NSURLSessionDelegate,
                                    NSURLSessionDataDelegate,
                                    NSURLSessionDownloadDelegate,
                                    NSURLSessionTaskDelegate>
@end

@implementation BJNetworkURLSession {
    /// The handler to be called when the request completes or error has occurs.
    BJNetworkURLSessionCompletionHandler _completionHandler;
...
- (instancetype)initWithNetworkLoggerDelegate:(id<BJNetworkLoggerProtocol>)networkLoggerDelegate {
  self = [super init];
  if (self) {
    ...
    NSArray *tempPathComponents = @[
      storageDirectory, kBJNetworkApplicationSupportSubdirectory, kBJNetworkTempDirectoryName
    ];
    _networkDirectoryURL = [NSURL fileURLWithPathComponents:tempPathComponents];
    _sessionID = [NSString stringWithFormat:@"%@-%@", kBJNetworkBackgroundSessionConfigIDPrefix,
...
+ (void)handleEventsForBackgroundURLSessionID:(NSString *)sessionID
                            completionHandler:
                                (BJNetworkSystemCompletionHandler)systemCompletionHandler {
  // The session may not be Analytics background. Ignore those that do not have the prefix.
  if (![sessionID hasPrefix:kBJNetworkBackgroundSessionConfigIDPrefix]) {
    return;
  }
  BJNetworkURLSession *fetcher = [self fetcherWithSessionIdentifier:sessionID];
  if (fetcher != nil) {
    [fetcher addSystemCompletionHandler:systemCompletionHandler forSession:sessionID];
  } else {
//    GULLogError(kGULLoggerNetwork, NO,
//                [NSString stringWithFormat:@"I-NET%06ld", (long)kBJNetworkMessageCodeNetwork003],
...
- (nullable NSString *)sessionIDFromAsyncPOSTRequest:(NSURLRequest *)request
  ...
  [self maybeRemoveTempFilesAtURL:_networkDirectoryURL
                     expiringTime:kBJNetworkTempFolderExpireTime];
  ...
  if (_backgroundNetworkEnabled && [self ensureTemporaryDirectoryExists]) {
    ...
    if (writeError) {
      [_loggerDelegate GULNetwork_logWithLevel:kBJNetworkLogLevelError
                                   messageCode:kBJNetworkMessageCodeURLSession000
  ...
  if (!session || !postRequestTask) {
    NSError *error = [[NSError alloc]
        initWithDomain:kBJNetworkErrorDomain
                  code:BJErrorCodeNetworkRequestCreation
              userInfo:@{kBJNetworkErrorContext : @"Cannot create network session"}];
...
- (void)URLSession:(NSURLSession *)session
  ...
  if (!url.path) {
    [_loggerDelegate
        GULNetwork_logWithLevel:kBJNetworkLogLevelError
                    messageCode:kBJNetworkMessageCodeURLSession001
  ...
  if (error) {
    [_loggerDelegate GULNetwork_logWithLevel:kBJNetworkLogLevelError
                                 messageCode:kBJNetworkMessageCodeURLSession002
...
- (void)URLSessionDidFinishEventsForBackgroundURLSession:(NSURLSession *)session
    API_AVAILABLE(ios(7.0)) {
  [_loggerDelegate GULNetwork_logWithLevel:kBJNetworkLogLevelDebug
                               messageCode:kBJNetworkMessageCodeURLSession003
...
- (void)URLSession:(NSURLSession *)session
  ...
  BJNetworkURLSessionCompletionHandler handler = _completionHandler;
  ...
  if (task.response) {
    ...
    error = [[NSError alloc]
        initWithDomain:kBJNetworkErrorDomain
                  code:BJErrorCodeNetworkInvalidResponse
              userInfo:@{kBJNetworkErrorContext : @"Network Error: Empty network response"}];
  }
...
- (void)URLSession:(NSURLSession *)session
  ...
  if ([challenge.protectionSpace.authenticationMethod
    ...
    if (serverTrust == NULL) {
      [_loggerDelegate GULNetwork_logWithLevel:kBJNetworkLogLevelDebug
                                   messageCode:kBJNetworkMessageCodeURLSession004
    ...
    if (!credential) {
      [_loggerDelegate GULNetwork_logWithLevel:kBJNetworkLogLevelWarning
                                   messageCode:kBJNetworkMessageCodeURLSession005
    ...
    [_loggerDelegate GULNetwork_logWithLevel:kBJNetworkLogLevelDebug
                                 messageCode:kBJNetworkMessageCodeURLSession006
    ...
    void (^callback)(BOOL) = ^(BOOL allow) {
      if (allow) {
        ...
        [self->_loggerDelegate
            GULNetwork_logWithLevel:kBJNetworkLogLevelDebug
                        messageCode:kBJNetworkMessageCodeURLSession007
    ...
    dispatch_async(evaluateBackgroundQueue, ^{
      ...
      @synchronized([BJNetworkURLSession class]) {
#pragma clang diagnostic push
...
#pragma clang dianostic pop
      ...
      if (trustError != errSecSuccess) {
        [self->_loggerDelegate GULNetwork_logWithLevel:kBJNetworkLogLevelError
                                           messageCode:kBJNetworkMessageCodeURLSession008
...
- (void)addSystemCompletionHandler:(BJNetworkSystemCompletionHandler)handler
                        forSession:(NSString *)identifier {
  if (!handler) {
    [_loggerDelegate
        GULNetwork_logWithLevel:kBJNetworkLogLevelError
                    messageCode:kBJNetworkMessageCodeURLSession009
...
- (void)callSystemCompletionHandler:(NSString *)identifier {
    BJMutableDictionary *systemCompletionHandlers =
      [[self class] sessionIDToSystemCompletionHandlerDictionary];
  BJNetworkSystemCompletionHandler handler = [systemCompletionHandlers objectForKey:identifier];
...
- (void)maybeRemoveTempFilesAtURL:(NSURL *)folderURL expiringTime:(NSTimeInterval)staleTime {
  ...
  if (error && error.code != NSFileReadNoSuchFileError) {
    [_loggerDelegate
        GULNetwork_logWithLevel:kBJNetworkLogLevelDebug
                    messageCode:kBJNetworkMessageCodeURLSession012
...
- (void)removeTempItemAtURL:(NSURL *)fileURL {
  ...
  if (![fileManager removeItemAtURL:fileURL error:&error] && error.code != NSFileNoSuchFileError) {
    [_loggerDelegate
        GULNetwork_logWithLevel:kBJNetworkLogLevelError
                    messageCode:kBJNetworkMessageCodeURLSession013
...
+ (instancetype)fetcherWithSessionIdentifier:(NSString *)sessionIdentifier {
  BJNetworkURLSession *session = [self sessionFromFetcherMapForSessionID:sessionIdentifier];
  if (!session && [sessionIdentifier hasPrefix:kBJNetworkBackgroundSessionConfigIDPrefix]) {
    session = [[BJNetworkURLSession alloc] initWithNetworkLoggerDelegate:nil];
...
+ (NSMapTable<NSString *, BJNetworkURLSession *> *)sessionIDToFetcherMap {
...
/// created.
+ (BJMutableDictionary *)sessionIDToSystemCompletionHandlerDictionary {
  static BJMutableDictionary *systemCompletionHandlers;

  static dispatch_once_t systemCompletionHandlerOnceToken;
  dispatch_once(&systemCompletionHandlerOnceToken, ^{
    systemCompletionHandlers = [[BJMutableDictionary alloc] init];
  });
...
- (BOOL)ensureTemporaryDirectoryExists {
  ...
  if (error && error.code != NSFileReadNoSuchFileError) {
    [_loggerDelegate
        GULNetwork_logWithLevel:kBJNetworkLogLevelWarning
                    messageCode:kBJNetworkMessageCodeURLSession014
  ...
  if (writeError) {
    [_loggerDelegate GULNetwork_logWithLevel:kBJNetworkLogLevelError
                                 messageCode:kBJNetworkMessageCodeURLSession015
...
- (void)excludeFromBackupForURL:(NSURL *)url {
  ...
  if (preventBackupError) {
    [_loggerDelegate GULNetwork_logWithLevel:kBJNetworkLogLevelError
                                 messageCode:kBJNetworkMessageCodeURLSession016
...
- (void)URLSession:(NSURLSession *)session
  ...
  NSArray *nonAllowedRedirectionCodes = @[
    @(kBJNetworkHTTPStatusCodeFound), @(kBJNetworkHTTPStatusCodeMovedPermanently),
    @(kBJNetworkHTTPStatusCodeMovedTemporarily), @(kBJNetworkHTTPStatusCodeMultipleChoices)
  ];
...
+ (void)setSessionInFetcherMap:(BJNetworkURLSession *)session forSessionID:(NSString *)sessionID {
  [[self sessionIDToFetcherMapReadWriteLock] lock];
  BJNetworkURLSession *existingSession =
  ...
  if (existingSession) {
    if (session) {
      ...
      [existingSession->_loggerDelegate GULNetwork_logWithLevel:kBJNetworkLogLevelInfo
                                                    messageCode:kBJNetworkMessageCodeURLSession019
                                                        message:message];
    }
...
+ (nullable BJNetworkURLSession *)sessionFromFetcherMapForSessionID:(NSString *)sessionID {
  [[self sessionIDToFetcherMapReadWriteLock] lock];
  BJNetworkURLSession *session = [[[self class] sessionIDToFetcherMap] objectForKey:sessionID];
  [[self sessionIDToFetcherMapReadWriteLock] unlock];
  return session;
}

- (void)callCompletionHandler:(BJNetworkURLSessionCompletionHandler)handler
                 withResponse:(NSHTTPURLResponse *)response
                         data:(NSData *)data
                        error:(NSError *)error {
  if (error) {
    [_loggerDelegate GULNetwork_logWithLevel:kBJNetworkLogLevelError
                                 messageCode:kBJNetworkMessageCodeURLSession017
